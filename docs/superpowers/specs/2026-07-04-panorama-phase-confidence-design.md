...
## Резолюция: пункт 3 всё же сделан (2026-07-05)

Пользователь столкнулся именно с тем сценарием, который пункт 3 предсказывал: на панораме
матрица (без реальной руды) массово красится в магнетит. Причина оказалась ровно в
описанном механизме — `segment_phases` пересчитывает 3-классовый Otsu-порог заново на
каждом 2048px-тайле; тайл, целиком состоящий из матрицы, не имеет настоящего
трёхмодального распределения яркости, и Otsu всё равно выдумывает ложное деление
mid/bright из собственного текстурного шума тайла. Дополнительно выяснилось, что
поканальный CLAHE (`preprocess`, применяется независимо на каждом тайле) сам по себе
локально растягивает контраст тайла и **сводит на нет** любой общий якорь, если сравнивать
с ним значения, уже прошедшие через CLAHE.

Риск, из-за которого пункт 3 был отложен («трогает функцию, от которой также зависит
`features.py`, F1 0.84 RF-классификатора»), снят конструктивно: `segment_phases` получил
**опциональный** параметр `levels` (`backend/app/shlif/segment.py`, см. новый
`compute_levels`). Без него поведение побитово не меняется — `features.py`,
`talc_detector.py`, `uncertainty.py`, close-up (`analyze.py`) и старый тест
`test_panorama_assemble.py::test_assemble_masks_partitions_every_pixel_exactly_once`
продолжают получать прежний, посегментный Otsu. Только `backend/app/pipeline/panorama.py`
теперь считает `(dark_t, bright_t)` один раз на секцию (даунскейл через
`masks.fit_max_side(..., EDIT_MAX_SIDE)`) и передаёт их в оба тайловых прохода
(`_assemble_masks`, `_run_panorama`'s classical fallback) через новый параметр `levels`.
Для самого якоря и для сегментационного входа тайлов CLAHE отключён
(`_segmentation_preprocess_cfg`) — WB и denoise остаются, чтобы якорь и тайлы были в одной
шкале яркости; talc-детекция и `extract_features` по-прежнему получают обычный
CLAHE'нутый `pre`, их эта правка не касается.

Тесты: `backend/tests/test_segment.py::test_matrix_only_tile_is_not_invented_into_magnetite_by_floating_otsu`
(vospроизводит баг на уровне `segment_phases` и подтверждает фикс через `levels`) и
`backend/tests/test_panorama_assemble.py::test_assemble_masks_with_anchor_keeps_ore_free_tiles_matrix`
(та же проверка через `_assemble_masks`, с включённым CLAHE в конфиге — именно тот сценарий,
где голого якоря без отключения CLAHE недостаточно).

Не сделано (осталось вне скоупа этого фикса): `uncertainty.py::ensemble_uncertainty` внутри
панорамы всё ещё гоняет `segment_phases` без якоря (свои 5 photometric-пертурбаций на
тайл) — влияет только на `low_conf_zones`/`undetermined_fraction`, не на видимую
раскраску фаз, но по той же логике на чисто-матричном тайле может не заметить проблему
(все 5 пертурбаций согласованно ошибутся одинаково). Кандидат на отдельный follow-up.
